...
const ATSScanner = () => {
  ...
  const handleScan = async (e) => {
    ...
    setLoading(true);
    setScanResult('');
    const toastId = toast.loading('Scanning Resume against Job Description...');

    try {
      const prompt = `Act as an expert ATS (Applicant Tracking System) scanner. I will provide a Job Description and my current Resume.
      ...
      Analyze my resume against the Job Description. Provide a detailed evaluation, including:
      ...
      if (res.data.success) {
        ...
        toast.error('Failed to scan resume.', { id: toastId });
      }
    } catch (err) {
      console.error(err);
      ...
      <div className="w-full max-w-7xl mx-auto flex flex-col lg:flex-row relative z-10 p-4 sm:p-6 lg:p-8 pt-24 min-h-screen gap-8">
        ...
        <div className="w-full lg:w-1/3 flex flex-col h-full">
            <h1 className="text-3xl font-bold text-white mb-2 drop-shadow-lg">
                ATS <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#10b981] to-teal-400">Scanner</span>
            </h1>
            <p className="text-gray-400 text-sm mb-6">
                Paste the target job description. We'll score your primary resume and highlight missing critical skills.
            </p>

            <form onSubmit={handleScan} className="bg-[#111827]/80 backdrop-blur-xl p-6 rounded-2xl shadow-2xl border border-[#1f2937]/50 flex flex-col flex-grow gap-4">
            ...
            <div className="bg-[#111827]/80 backdrop-blur-xl p-8 rounded-2xl shadow-2xl border border-[#1f2937]/50 h-full min-h-[600px] overflow-y-auto">
                ...
                {scanResult ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-gray-500 opacity-50 pb-20">
                        <svg className="w-16 h-16 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        <p className="text-lg">Your ATS compatibility score will appear here.</p>
                    </div>
                )}
            </div>
        </div>
